/* rfid.c
 * This file contains the  function definitions of the rfid api
 * 20,April 2024
 *  REFERENCE- https://blog.embeddedexpert.io/?p=768
...
void rc522_init(void) {
	 ...
	 * */
	SPI_Init();
	GPIOE->MODER |= GPIO_MODER_MODE7_0; // enable the gpio pins for reset and chip select
	GPIOE->MODER &= ~GPIO_MODER_MODE7_1;
	GPIOE->MODER |= GPIO_MODER_MODE4_0;        //nss pin
	...
	rc522_reset();      //configure the rfid reader
...
bool rc522_check_card(uint8_t *id) {
	...
	if (status == true) {
		...
		// printf("rc522_antiColl status: %d\n", status);
	}
...
bool rc522_toCard(uint8_t command, uint8_t *sendData, uint8_t sendLen,
